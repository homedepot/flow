...
class Cloud(metaclass=ABCMeta):
    ...
    def download_custom_deployment_script(self, custom_deploy_script):
        ...
        if 'github' in custom_deploy_script:
            commons.print_msg(Cloud.clazz, method, "Looking for deploy script from GitHub {}"
            ...
            try:
                if os.getenv("GITHUB_TOKEN"):
                    ...
                    commons.print_msg(Cloud.clazz, 'No GITHUB_TOKEN detected in environment. Attempting to access '
                                                  'deploy script anonymously.', 'WARN')
                    resp = requests.get(custom_deploy_script, timeout=self.http_timeout)

            except:
                commons.print_msg(Cloud.clazz, method, "Failed retrieving custom deploy script from GitHub {}".format(
            ...
            if resp.status_code == 200:
                ...
                commons.print_msg(Cloud.clazz, method, resp.text)
            else:
                commons.print_msg(Cloud.clazz, method, "Failed retrieving custom web deploy script from {script}. "
            ...
            commons.print_msg(Cloud.clazz, method, "Looking for deploy script from web at {}"
                              .format(custom_deploy_script))
            ...
            commons.print_msg(Cloud.clazz, method, ("Looking for deploy script locally", custom_deploy_script))

            if not os.path.isfile(custom_deploy_script.strip()):
                commons.print_msg(Cloud.clazz, method, ("Failed retrieving custom deploy script locally from {}"
    ...
    def find_deployable(self, file_ext, dir_to_look_in):
        ...
        commons.print_msg(Cloud.clazz, method, "Looking for a {ext} in {dir}".format(ext=file_ext, dir=dir_to_look_in))

        deployable_files = commons.get_files_of_type_from_directory(file_ext.lower(), dir_to_look_in)

        if len(deployable_files) > 1:
            commons.print_msg(Cloud.clazz, method, "Found more than 1 artifact in {}".format(dir_to_look_in), 'ERROR')
            # raise IOError('Found more than 1 artifact')
            exit(1)
        elif len(deployable_files) == 0:
            commons.print_msg(Cloud.clazz, method, "Could not find file of type {ext} in {dir}".format(
    ...
    def run_deployment_script(self, custom_deploy_script):
        ...
        if execute_custom_script.returncode != 0:
            commons.print_msg(Cloud.clazz, method, "Failed calling {command}. Return code of {rtn}".format(
